...
/*   Updated: 2023/07/10 22:47:11 by jthuysba         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	remov_quote_bis(t_substitution *s, int i, t_minishell *data)
{
	while (ft_isalnum(s->keep_var[i]))
		i++;
	s->end = i;
	s->keep_var2 = ft_strdup_size(s->keep_var, s->end);
	s->var_substitute = substitution(s->keep_var2, data);
	free(s->keep_var2);
}
